...
for product_id in range(1, 151):
    ...
    if scenario < 0.15: #15% critical - out of stock or very low
        ...
        qty = random.randint(1, product['safety_stock'])
    elif scenario < 0.50: # 20% below reorder point
        qty = random.randint(product['safety_stock'], product['reorder_point'])
        ...
        inventory.append({
            ...
            'last_counted': (datetime.now() - timedelta(days = random.randinit(0, 30))).strftime('%Y-%m-%d'),
...
for i in range(100):
    product_id = random.randint(1, 150)
    product = df_products[df_products['product_id'] == product.id].iloc[0]

    order_date = (datetime.now() - timedelta(days = random.randint(0, 180))).strftime('%Y-%m-%d')
    expected_delivery = (datetime.strptime(order_date, '%Y-%m-%d') + timedelta(days = product['lead_time_days'])).strftime('%Y-%m-%d')
